...
@section('content')
    <div class="space-y-6" x-data="reportBuilder()">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Edit Custom Report</h1>
                <p class="mt-1 text-sm text-gray-600">{{ $customReport->report_code }} - {{ $customReport->title }}</p>
            </div>
            <a href="{{ route('admin.reports.custom.show', $customReport) }}"
        ...
        <form action="{{ route('admin.reports.custom.update', $customReport) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="title" value="{{ old('title', $customReport->title) }}" required
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description" rows="3"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#0053C5] focus:border-transparent">{{ old('description', $customReport->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <select name="report_type" required
                            ...
                            @foreach ($reportTypes as $value => $label)
                                <option value="{{ $value }}"
                                    {{ old('report_type', $customReport->report_type) == $value ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select name="event_id"
                            ...
                            @foreach ($events as $event)
                                <option value="{{ $event->id }}"
                                    {{ old('event_id', $customReport->event_id) == $event->id ? 'selected' : '' }}>
                                    {{ $event->title }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="date" name="period_start"
                            value="{{ old('period_start', $customReport->period_start?->format('Y-m-d')) }}"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#0053C5] focus:border-transparent">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" name="period_end"
                            value="{{ old('period_end', $customReport->period_end?->format('Y-m-d')) }}"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#0053C5] focus:border-transparent">
                    </div>
                </div>
            </div>
                        ...
                        @foreach ($dataSources as $key => $label)
                            <label
                                ...
                                <input type="checkbox" name="data_sources[]" value="{{ $key }}"
                                    {{ in_array($key, old('data_sources', $customReport->data_sources ?? [])) ? 'checked' : '' }}
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select name="chart_config[type]"
                                ...
                                @foreach ($chartTypes as $key => $label)
                                    <option value="{{ $key }}"
                                        {{ old('chart_config.type', $customReport->chart_config['type'] ?? '') == $key ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="color" name="chart_config[color]"
                                value="{{ old('chart_config.color', $customReport->chart_config['color'] ?? '#0053C5') }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="visibility" required
                            ...
                            @foreach ($visibilities as $value => $label)
                                <option value="{{ $value }}"
                                    {{ old('visibility', $customReport->visibility) == $value ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="status" required
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#0053C5] focus:border-transparent">
                            <option value="draft"
                                {{ old('status', $customReport->status) == 'draft' ? 'selected' : '' }}>
                                Draft
                            </option>
                            <option value="saved"
                                {{ old('status', $customReport->status) == 'saved' ? 'selected' : '' }}>
                                Tersimpan
                            </option>
                            <option value="published"
                                {{ old('status', $customReport->status) == 'published' ? 'selected' : '' }}>
                                Dipublikasikan
                            </option>
                        </select>
                    </div>
                    ...
                    <div class="md:col-span-2">
                        <label class="flex items-center gap-2 mb-4">
                            <input type="checkbox" name="is_scheduled" value="1"
                                {{ old('is_scheduled', $customReport->is_scheduled) ? 'checked' : '' }}
                                class="w-4 h-4 text-[#0053C5] border-gray-300 rounded focus:ring-[#0053C5]"
                                x-model="isScheduled">
                            ...
                            <div>
                                ...
                                <select name="schedule_frequency"
                                    ...
                                    @foreach ($frequencies as $value => $label)
                                        <option value="{{ $value }}"
                                            {{ old('schedule_frequency', $customReport->schedule_frequency) == $value ? 'selected' : '' }}>
                                            {{ $label }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="md:col-span-2">
                        <label class="flex items-center gap-2">
                            <input type="checkbox" name="regenerate" value="1"
                                class="w-4 h-4 text-[#0053C5] border-gray-300 rounded focus:ring-[#0053C5]">
                            <span class="text-sm font-medium text-gray-700">Regenerate report setelah update</span>
                        </label>
                        <p class="text-xs text-gray-500 ml-6 mt-1">Report akan di-regenerate dengan konfigurasi baru</p>
                    </div>
                </div>
            </div>
...
@push('scripts')
    <script>
        function reportBuilder() {
            return {
                isScheduled: {{ $customReport->is_scheduled ? 'true' : 'false' }},
                filters: @json($customReport->filters ?? []),
                metrics: @json($customReport->metrics ?? []),
                dimensions: @json($customReport->dimensions ?? []),
