...
@section('title', 'Buat Laporan Akhir Acara')
...
@section('content')
    <div class="space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Buat Laporan Akhir Acara</h1>
                <p class="mt-1 text-sm text-gray-600">Buat laporan akhir untuk acara yang telah selesai</p>
            </div>
        ...
        <form action="{{ route('admin.reports.final-event-reports.store') }}" method="POST" class="space-y-6">
            @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title') }}" required
                            ...
                            placeholder="Contoh: Laporan Akhir Seminar Motivasi Ramadhan 2024">
                        @error('title')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                ...
                <textarea name="notes" rows="4"
                    ...
                    placeholder="Tambahkan catatan tambahan di sini...">{{ old('notes') }}</textarea>
            </div>
...
@push('scripts')
    <script>
        // Auto-generate title based on selected event
        document.getElementById('event_id').addEventListener('change', function() {
            const selectedOption = this.options[this.selectedIndex];
            const eventName = selectedOption.text.split(' - ')[0];
            const titleInput = document.getElementById('title');

            if (titleInput.value === '' && eventName) {
                titleInput.value = 'Laporan Akhir ' + eventName;
            }
        });
    </script>
@endpush
