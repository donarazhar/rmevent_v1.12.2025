...
@section('title', 'Event dengan Tag: ' . $tag->name)
    ...
    <section class="bg-gradient-to-r from-purple-600 to-purple-800 text-white py-12">
        <div class="container mx-auto px-4">
            <div class="max-w-7xl mx-auto">
                {{-- Breadcrumb --}}
                <nav class="flex items-center space-x-2 text-sm text-purple-100 mb-6">
                    <a href="{{ route('home') }}" class="hover:text-white">Beranda</a>
                    <span>/</span>
                    <a href="{{ route('events.index') }}" class="hover:text-white">Event</a>
                    <span>/</span>
                    <span class="text-white font-semibold">Tag: {{ $tag->name }}</span>
                </nav>

                <div class="flex items-center mb-4">
                    <div class="flex items-center space-x-3">
                        <span class="px-6 py-3 rounded-2xl text-lg font-bold"
                            style="background-color: {{ $tag->color }}; color: #ffffff;">
                            #{{ $tag->name }}
                        </span>
                        <span class="text-purple-100">{{ $events->total() }} event</span>
                    </div>
                </div>

                <p class="text-lg text-purple-100">
                    Event yang berkaitan dengan topik <strong>{{ $tag->name }}</strong>
                </p>
            </div>
        </div>
    </section>
                            ...
                            <a href="{{ route('events.show', $event->slug) }}"
                                ...
                                <div class="p-6">
                                    ...
                                    <div class="flex flex-wrap gap-2 mb-4">
                                        @foreach ($event->tags->take(3) as $eventTag)
                                            <span class="px-2 py-1 text-xs rounded-lg"
                                                style="background-color: {{ $eventTag->color }}20; color: {{ $eventTag->color }};">
                                                #{{ $eventTag->name }}
                                            </span>
                                        @endforeach
                                    </div>
                ...
                @else
                    ...
                    <div class="text-center py-16">
                        ...
                        <p class="text-gray-600 mb-6">
                            Saat ini belum ada event dengan tag <strong>#{{ $tag->name }}</strong>. <br>
